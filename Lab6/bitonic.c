...
void runKernel(cl_kernel kernel, int threads, cl_mem data, unsigned int outer, unsigned int inner)
{
	...
	ciErrNum |= clSetKernelArg(kernel, 1, sizeof(cl_uint), (void *) &outer);
  ciErrNum |= clSetKernelArg(kernel, 2, sizeof(cl_uint), (void *) &inner);
...
int bitonic_gpu(unsigned int *data, unsigned int length)
{
	...
	// ********** RUN THE KERNEL ************
 ResetMilli();  
    for(int k = 2; k <= length; k = 2 * k){
        for(int j = k/2; j >= 1; j = j/2) {
            runKernel(gpgpuSort, length, io_data, k, j);
        }
    }
